...
class Pic
{
    ...
    public function imageInterLace($interlace = null)
    {
        if('jpeg' !== $this->imgInfo['ext'])
            $this->transformImage('jpeg');
        imageinterlace($this->imgRes, $interlace);
    }
     ...
     * @throws Exception
     */
    public function transformImage($to_ext, $save_path = null)
    {
        if($to_ext === $this->imgInfo['ext'])return;
        $function = 'image' . $to_ext;
        if(!function_exists($function))throw new cuitException("服务器不支持{$function}函数", 1);
        if ($save_path) {
            return $function($this->imgRes, $save_path);
        } else {
            $tmp = '/tmp/' . md5(uniqid(microtime(true),true)) . '.' . $to_ext;
            if ($function($this->imgRes, $tmp)) {
    ...
    // 输出图片
    public function outPic($filepath = null)
    {
        $info = $this->imgInfo;
        ...
        if($filepath)
        {
            return $fun($im, $filepath, $this->quality);
        }
    ...
    public static function onlineSave($url, $dir, $ref)
    {
        ...
        if (intval(filesize($temp_path)) === $originalSize) {
            ...
            if (strlen($suffix) === 0) {
                $ret['errorCode'] = $ret['status'] = 28;
                $ret['msg'] = "下载出错";
            } else {
                $new_path = $dir . "/{$md5}.{$suffix}";
                if (file_exists($new_path)) {
                    $ret['errorCode'] = $ret['status'] = 200;
                    $ret['msg'] = "文件已存在";
                    $ret['path'] = $new_path;
                } else {
            ...
            if (!($fp = fopen(trim($url), "rb"))) {
                $ret['errorCode'] = $ret['status'] = 26;
                $ret['msg'] = "远程文件读取失败";
            } elseif (!($fp2 = fopen($fpath = "{$dir}/{$fname}", "wb"))) {
                $ret['errorCode'] = $ret['status'] = 27;
                $ret['msg'] = "本地文件打开失败";
            } else {
                while (!feof($fp)) {
